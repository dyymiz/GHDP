...
DP_fn.compact.consistency.check <- function(parm)
{err <- 0
...
DP_fn.check.nbhd <- function(parm)
{
...
DP_fn.consistency.check <- function(parm)
{err <- 0
...
DP_fn.swap.clusters <- function(parm, g1, g2)
{
...
DP_fn.clip.clusters <- function(parm, keep)
{
...
DP_fn.log.lik <- function(gg, x.mt, parm, colSums=TRUE)
{
  if (gg > 0)
...
DP_fn.nbhd <- function(relative_I, parm, max.col.nbhd.size)
{
...
DP_fn.post.prob.and.delta <- function(parm, max.col.nbhd.size, col.frac.probes) {
  ...
  {subset_log.ss.mt[(gg+1),] <- DP_fn.log.lik(gg, x.mt = parm$X[,col.subset], parm)
  }
  ...
  while (length(relative_I) >= 1) {
    tmp <- DP_fn.nbhd(relative_I, parm, max.col.nbhd.size) # HOT inside function
    relative_k <- tmp[[1]]
    relative_I.k <- tmp[[2]]
    relative_I <- tmp[[3]]
    #
...
DP_fn.gibbs <- function(k, parm, data)
{	k <- parm$k
...
L.v <- sapply(0:parm$clust$G, DP_fn.log.lik, x.mt, parm) # HOT
...
if (length(emptied.indx) >0)
{log.prior.v[-(emptied.indx+1)] <- log(c((parm$b0+parm$clust$C.m0), (parm$clust$C.m.vec[-emptied.indx]), (parm$b1+new.G)))
log.prior.v[emptied.indx+1] <- -Inf
}

if (length(emptied.indx) ==0)
{log.prior.v <- log(c((parm$b0+parm$clust$C.m0), (parm$clust$C.m.vec), (parm$b1+new.G)))
}
...
DP_fn.fast_col <- function(cc, parm, data)
{
  ...
  spread.mass <- (parm$b1+new.G)/(1+length(emptied.indx))
  
  if (length(emptied.indx) >0)
  {
    log.prior.v[-(emptied.indx+1)] <- log(c((parm$b0+parm$clust$C.m0), (parm$clust$C.m.vec[-emptied.indx]), spread.mass))
    log.prior.v[emptied.indx+1] <- log(spread.mass)
  }
  
  if (length(emptied.indx) ==0)
  {log.prior.v <- log(c((parm$b0+parm$clust$C.m0.k.comp), (parm$clust$C.m.vec.k.comp), spread.mass))
  }
  ...
  if (!exit) # CONTINUE W/O EXITING FUNCTION
  {
    ...
    new.log.lik <- new.log.lik + sum(DP_fn.log.lik(gg, x.mt=x_gg.mt, parm))
    }
    ...
    {indx.gg <- old.c.k==gg
    x_gg.mt <- matrix(parm$X[,I.k[indx.gg]], ncol=sum(indx.gg))
    old.log.lik <- old.log.lik + sum(DP_fn.log.lik(gg, x.mt=x_gg.mt, old.parm))
    }
    # 
...
DP_fn.drop <- function(parm, computeMode)
{
  ...
  if (parm$clust$G > 0)
  {
    if (num.dropped > 0)
    {
      for (rr in 1:num.dropped)
      {
        ...
        parm <- DP_fn.swap.clusters(parm, g1 = new.label, g2 = old.label)
      }
    }
    ...
    keep <- 1:parm$clust$G
    parm <- DP_fn.clip.clusters(parm, keep)
...
fast_DP_fn.main <- function(parm, data, col.frac.probes, prob.compute.col.nbhd, max.col.nbhd.size)
{
  p <- parm$p
  ...
  if (is.null(parm$clust$col.nbhd.k) | col_flip){
    parm <- DP_fn.post.prob.and.delta(parm, max.col.nbhd.size, col.frac.probes)
  }
  ...
  { tmp <- DP_fn.gibbs(k=parm$k, parm, data)
  parm <- tmp[[1]]
  ...
  if (length(parm$clust$col.nbhd[[cc]])>1)
  {
    tmp <- DP_fn.fast_col(cc, parm, data)
    parm <- tmp[[1]]
    new.flag.v <- c(new.flag.v, tmp[[2]])
  ...
  parm <- DP_fn.drop(parm)
  
  # now drop empty elementwise clusters
  
  parm <- element_fn.drop(parm)
  
  err <- DP_fn.compact.consistency.check(parm)
  if (err > 0)
