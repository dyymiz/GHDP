...
fn.init.clusters <- function(s, t, m, k, p, n, r, parm)
{
  ...
  # unsupervised case
  options(warn=0)
  tmp2 <- kmeans(X.mt, iter.max=1000, centers=k, nstart=2)
  options(warn=2)
...
# }
# 
fn.G_0 <- function(alpha, parm, true_parm, s) {
...
# this maybe wrong
fn.G_mt <- function(alpha, parm, true_parm){
...
fn.eda <- function(s, t, m, k, p, n, r, parm, true_parm, data)
{
  
  parm <- fn.init.clusters(s, t, m, k, p, n, r, parm)
...
fn.gen.clust <- function(s, t, m, k, p, n, r, parm, true_parm, data, max.row.nbhd.size, row.frac.probes, col.frac.probes)
{
  ...
  parm$G.new <- round(parm$p/6)
  parm <- fn.eda(s, t, m, k, p, n, r, parm, true_parm, data)
  
  #################
  
  parm <- fn.hyperparameters.init(data, parm, t)
...
fn.init <- function(s, t, m, k, p, n, r, true, data, max.row.nbhd.size, row.frac.probes, col.frac.probes, true_parm, tBB_flag)
{
  ...
  parm$a.R <- 10 # true$a.R
  
  # mass paramater of columns
  parm$b1 <- 10 # 0.001 # true$b1
  ...
  parm$shift <- 1e-04
  parm <- fn.gen.clust(s, t, m, k, p, n, r, parm, true_parm, data, max.row.nbhd.size, row.frac.probes, col.frac.probes)
...
fn.gen.tau.init  <- function(data, parm, t)
{
  ###################
  # update tau
  ###################
...
fn.hyperparameters.init <- function(data, parm, t)
{
  
  # also updates update tau_int
  parm <- fn.gen.tau.init(data, parm, t)
...
fn.save.all <- function(s, t, m, k, p, n, r, parm, all_parm, data){
...
fn.comb <- function(s, t, m, k, p, n, r, parm, all_parm, data, mm){
...
fn.postBurnin <- function(s, t, m, k, p, n, r, text, tmp, All.Stuff, offset, n.reps, data, data1, parm.m,all_parm, dahl.flag, max.row.nbhd.size, max.col.nbhd.size, row.frac.probes, col.frac.probes, prob.compute.col.nbhd, true_parm, prop.X.miss)
  {
  ...
  for (cc in 1:n.reps){
    ...
    parm.m <- fn.G_0(alpha,parm.m, true_parm, s)
    ...
    for(mm in 1:s){
      ...
      for(tt in 1:t){
        ...
        XX[[mm]][[tt]] <- data$X
        data <- fn.data.slice(data$X,parm,true_parm,mm,tt,prop.X.miss)$data1
        tmp_parm.m[[mm]] <- fn.data.slice(data$X,parm,true_parm,mm,tt,prop.X.miss)$tmp_parm
        tmp_parm <- tmp_parm.m[[mm]]
        tmp_parm$X <- data$X
        tmp_parm$p <- dim(tmp_parm$X)[2]
        
        tmp_parm <- fn.G_mt(alpha, tmp_parm, true_parm)
      ...
      all_parm <- fn.comb(s, t, m, k, p, n, r, parm, all_parm, data)
      
      all_parm <- row_PDP_fn.main(all_parm, data, col.frac.probes, prob.compute.col.nbhd, max.col.nbhd.size)
      
      parm <- fn.save.all(s, t, m, k, p, n, r, parm, all_parm, data)
  ...
  if (cc %% 100 == 0)
    ...
    print(paste("row cluster #", max(all_parm$est_c.v)))
    print(paste("column cluster #", max(parm$clust$c.v[[1]])))
    }
  ...
  All.Stuff$RI <- lapply(1:s,function(x) adjustedRandIndex(true_parm$clust$r.v[[x]], All.Stuff$est_c.v[[x]]))
  ...
  All.Stuff$RI.c <- lapply(1:s,function(x) lapply(1:t, function(y) adjustedRandIndex(true_parm$clust$c.v[[x]][[y]], All.Stuff$est_c.v.c[[x]][[y]])))
...
fn.mcmc <- function(s, t, m, k, p, n, r, true, data, data1, n.burn, n.reps, max.row.nbhd.size, max.col.nbhd.size, row.frac.probes, col.frac.probes, prob.compute.col.nbhd, true_parm, dahl.flag=FALSE,
                    tBB_flag=FALSE, prop.X.miss=0)
	{
  ...
  parm.m[[mm]] <- fn.init(s, t, m, k, p, n, r, true, data, max.row.nbhd.size, row.frac.probes, col.frac.probes, true_parm, tBB_flag)
  ...
  G <- array(,c(s,n.burn))
	XX <- A <- C <- R <- rep(list(list()),t)
	 ...
	 for (cc in 1:n.burn){
	  ...
	  if (cc %% 100 == 0)
	  {print(paste(text, "REPS = ",cc,date(),"***********"))
	    print(all_parm$clust$G)
	    print(max(parm$clust$r.v))
	   # print(paste("G = ",G[,cc]))
	   # print(paste("R_sq = ", unlist(R_sqe)))
	   # print(paste("tau = ", parm$tau))
	  }
	...
	tmp_1 = fn.postBurnin(s, t, m, k, p, n, r, text, tmp, All.Stuff, offset=0, n.reps, data, data1, parm.m,all_parm, dahl.flag=FALSE, max.row.nbhd.size, max.col.nbhd.size, row.frac.probes, col.frac.probes, prob.compute.col.nbhd, true_parm, prop.X.miss)
  parm.m = tmp_1$parm.m
  ...
  # parm$min_Dahl_dist <- parm$Dahl_dist <- rep(list(list()),t)
	...
	tmp_1 = fn.postBurnin(s, t, m, k, p, n, r, text, tmp, All.Stuff, offset=n.reps, n.reps, data, data1, parm.m,all_parm, dahl.flag=TRUE, max.row.nbhd.size, max.col.nbhd.size, row.frac.probes, col.frac.probes, prob.compute.col.nbhd, true_parm, prop.X.miss)
	parm.m = tmp_1$parm.m
	All.Stuff = tmp_1$All.Stuff
	all_parm.m = tmp_1$all_parm.m
