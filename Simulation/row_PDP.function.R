...
row_PDP_fn.fast_col <- function(cc, parm, data)
{
  ...
  for (tt in 1:length(x.mt))
  {
    ...
    ### changed 
    if(sum(tmp.marg.v)==0) tmp.marg.v <- rep(1e-03, length(tmp.marg.v))
  ...
  if (length(emptied.indx) >0)
  {
    log.prior.v[-(emptied.indx+1)] <- log(c(0, (parm$clust$C.m.vec[-emptied.indx]-parm$d), 0))#spread.mass))
  ...
  if (length(emptied.indx) ==0)
  {log.prior.v <- log(c(0, (parm$clust$C.m.vec.k.comp-parm$d), 0))#spread.mass))
