...
PDP_fn.log.lik <- function(gg, x.mt, parm, colSums=TRUE)
	...
	if (gg > 0)
		...
		if (sum(z.g.v) > 0)
			...
			tau <- c(parm$tau/sqrt(parm$clust$R.m.vec))
...
PDP_fn.post.prob.and.delta <- function(parm, max.col.nbhd.size, col.frac.probes) {
...
{	k <- parm$k
  ...
  # allow -Inf's in Gibbs sampling log-prior (just emptied clusters)
	if (length(emptied.indx) >0)
		{log.prior.v[-(emptied.indx+1)] <- log(c(0, (parm$clust$C.m.vec[-emptied.indx]-parm$d), (parm$b1+new.G*parm$d)))
		 # log(c((parm$b0+parm$clust$C.m0), (parm$clust$C.m.vec[-emptied.indx]-parm$d), (parm$b1+new.G*parm$d)))
		log.prior.v[emptied.indx+1] <- -Inf
		}

	if (length(emptied.indx) ==0)
		{log.prior.v <- log(c(0, (parm$clust$C.m.vec-parm$d), (parm$b1+new.G*parm$d)))
...
PDP_fn.fast_col <- function(cc, parm, data)
{
...
fast_PDP_fn.main <- function(parm, data, col.frac.probes, prob.compute.col.nbhd, max.col.nbhd.size)
{
  # XX is n by p
  # parm$X is R by p, parm$clust$A.mt/s.mt R by G
  # parm$X is n by p at the end
  XX <- parm$X
  parm$X <- array(,c(parm$clust$R, parm$p))
     ...
     for (cc in parm$subset_nbhd.indx)
      ...
      { tmp <- PDP_fn.gibbs(k=parm$k, parm, data)
        ...
        new.flag.v <- c(new.flag.v, tmp[[2]])
			...
			if (length(parm$clust$col.nbhd[[cc]])>1)
			{
			  tmp <- PDP_fn.fast_col(cc, parm, data)
			  parm <- tmp[[1]]
			  new.flag.v <- c(new.flag.v, tmp[[2]])
