...
# 
#   for (ss in 1:s) {
#   All.Stuff.s$parm$X[[ss]] <- All.Stuff$parm.m[[ss]]$X
#   tmp_u.phi <- apply(t(All.Stuff$all_parm.m[[ss]]$clust$A.mt),1,unique)
#   m <- max(All.Stuff$parm.m[[ss]]$clust$c.v)
#   All.Stuff.s$parm$est_c.v[[ss]] <- matrix(unlist(lapply(1:m,function(x) match(t(All.Stuff$all_parm.m[[ss]]$clust$A.mt)[x,],tmp_u.phi[[x]]))),byrow = TRUE,nrow = m)
#   All.Stuff.s$parm$clust$G[[ss]] <- lapply(1:m,function(x) length(tmp_u.phi[[x]]))
#   # All.Stuff.s$parm$clust$A.mt[[ss]] <- t(All.Stuff$all_parm.m[[ss]]$clust$A.mt)
#   All.Stuff.s$parm$clust$r.m.vec[[ss]] <- All.Stuff$parm.m[[ss]]$clust$C.m.vec
#   All.Stuff.s$parm$clust$r.v[[ss]] <- All.Stuff$parm.m[[ss]]$clust$c.v
#   
#   }
#   
#   All.Stuff.s
...
trueY$Y <- log(clinical.slt$days) #[n_sub]
# all uncensored delta
# trueY$delta <- rep(1, n2)
trueY$delta <- clinical.slt$censor #[n_sub]
